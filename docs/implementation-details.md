...
The _release_ profile is intended for production within the docker container. It uses structured JSON logging for STACKIT.

Required configuration via environment variables:

* `API_KEY` - API key for the services (to be passed as HTTP header `X-API-KEY`)
* `OPENAI_BASE_URL` - URL of the OpenAI-compatible server for translations

### `cargo run`

The _dev_ (debug) profile is intended for local development and uses human-readable colored logging.
It is ready to go for local development and testing. All configuration is optional.

It uses mock backends for the translation and management APIs if not configured.

## Agentic Coding

This project contains a set of agentic rules, skills and workflows in at [`.agent/`](.agent) to support the setup and development process.

It utilizes specific agentic workflows [`.agent/workflows`](.agent/workflows) to assist with onboarding, installation, and GitHub repository configuration. These workflows are designed to be run by AI coding assistants (like Google Antigravity) to guide new developers.
